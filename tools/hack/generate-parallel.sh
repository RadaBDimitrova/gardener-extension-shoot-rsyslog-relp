...
PROJECT_ROOT="${REPO_ROOT:-${CURRENT_DIR}/..}"
...
package_changed() {
  ...
  if [[ "$package" != "$current_module"/* ]]; then
    ...
    git diff --quiet "$DEFAULT_BRANCH" -- go.mod 2>/dev/null
    return $?
  else
    # For internal packages, check if package directory changed
    local pkg_path="${package#$current_module/}"
    git diff --quiet "$DEFAULT_BRANCH" -- "$pkg_path" 2>/dev/null
    return $?
  fi
}
...
should_generate_api_docs() {
  ...
  for pkg in $packages; do
    local pkg_path="${pkg#$MODULE_NAME/}"
    if ! git diff --quiet "$DEFAULT_BRANCH" -- "$pkg_path" 2>/dev/null; then
      return 0
    fi
  done
...
echo "$ROOTS" | while IFS= read -r dir; do
  ...
  if [ "$has_only_skippable" = true ] && [ "$has_skippable" = true ]; then
    ...
    if [ "$needs_gen" = true ]; then
      ...
      echo "Skipping $MODULE_NAME/$dir (no changes detected)" >&2
    fi
  else
