...
run_target() {
  ...
  case "$target" in
    ...
    codegen)
      ...
      $TOOLS_HACK_DIR/update-codegen.sh --groups "$CODEGEN_GROUPS" --mode "$mode"
      ;;
    manifests)
      local which=()
      ...
      if [[ "$mode" == "sequential" ]]; then
        ...
        $TOOLS_HACK_DIR/generate-sequential.sh $(overwrite_paths "${which[@]}")
      elif [[ "$mode" == "parallel" ]]; then
        $TOOLS_HACK_DIR/generate-parallel.sh "${which[@]}"
      else
