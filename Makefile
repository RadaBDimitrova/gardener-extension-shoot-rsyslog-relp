...
LD_FLAGS                    := "-w $(shell EFFECTIVE_VERSION=$(EFFECTIVE_VERSION) bash $(TOOLS_HACK_DIR)/get-build-ld-flags.sh k8s.io/component-base $(REPO_ROOT)/VERSION $(EXTENSION_PREFIX)-$(NAME))"
...
SKAFFOLD_BINARY := $(EXTENSION_PREFIX)-$(NAME)
SKAFFOLD_CONFIG := extension
DEFAULT_MANIFESTS_DIRS := charts cmd example imagevector pkg test
GENERATE_WHAT				  	:= protobuf manifests logcheck
...
install:
	@LD_FLAGS=$(LD_FLAGS) \
	bash $(TOOLS_HACK_DIR)/install.sh ./cmd/...
